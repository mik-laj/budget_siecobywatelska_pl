Feature: Category API
  In order to maintain the category shown on the site and allow other systems to integrate with us
  As an API user
...
#    And the "type" property is contain "/api/docs/errors#invalid_body_format"
  ...
  Scenario: Proper 404 exception on no category
    When I request "GET /api/categories/44999444"
...
#    And the "detail" property is equal "The programmer fake does not exist!"
  ...
  Scenario: GET one category
    When I request "GET /api/categories/2"
  ...
  Scenario: GET a collection of categories
