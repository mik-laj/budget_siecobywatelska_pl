    ...
    public function iRequest($httpMethod, $resource)
    {
        ...
        try {
            ...
            if($this->requestPayload){
                ...
                $request_option['body'] = $payload;
            }

            if ($this->authUser) {
                $request_option['auth'] = [$this->authUser, $this->authPassword];
            }

            $request_option['headers'] = array(
                'Accept' => 'application/json',
                'Content-Type' => 'application/json'
            );

            if($this->headers && count($this->headers) >= 1) {
                foreach ($this->headers as $key => $val) {
                    $request_option['headers'][$key] = $val;
                }
            }

            $this->response = $this
                ->client
                ->request($method, $resource, $request_option);
...
//            $this->response = $this->lastRequest->send();
     ...
     * @Given /^the "([^"]*)" header is "([^"]*)"$/
     */
    public function theHeaderIs($headerName, $expectedHeaderValue)
    {
        $response = $this->getResponse();
    ...
    /**
     * @Given /^the "([^"]*)" header is exist$/
     */
    public function theHeaderIsExist($headerName)
    {
        $response = $this->getResponse();
    ...
    public function thePropertyEquals($property, $expectedValue)
    {
        ...
        assertEquals(
            $expectedValue,
            $actualValue,
            "Asserting the [$property] property in current scope equals [$expectedValue|$actualValue]: "
        );
    }

    /**
     * @Then /^the "([^"]*)" property is contain "([^"]*)"$/
     */
    public function thePropertyIsContain($property, $expectedValue)
    {
        $payload = $this->getScopePayload();
        $actualValue = $this->arrayGet($payload, $property);
    ...
    /**
     * @Given /^the "([^"]*)" property is not contain "([^"]*)"$/
     */
    public function thePropertyIsNotContain($property, $expectedValue)
    {
        $payload = $this->getScopePayload();
        $actualValue = $this->arrayGet($payload, $property);
    ...
    /**
     * @Then /^the "([^"]*)" property is not exist$/
     */
    ...
    /**
     * @Then /^the "([^"]*)" property is contain (\d+) item(?:|s)$/
     */
    ...
    /**
     * @Then /^the "([^"]*)" property is an integer equalling "([^"]*)"$/
     */
    ...
    /**
     * @Then /^scope into the first embedded item$/
     */
    public function scopeIntoTheFirstEmbeddedItem()
    {
        $this->scope = "_embedded.items.0";
    }
...
//            'battles' => new EntityLookup($this->getProjectHelper()->getProgrammerRepository(), 'id'),
     ...
     *
     * @Given /^the link "([^"]*)" should exist and its value is "([^"]*)"$/
     */
    ...
    {
        $this->thePropertyEquals(
            sprintf('_embedded.items.%s.%s', $embeddedName, $property),
            $value
        );
    }

    /**
     * @Then /^the embedded list should contain (\d+) item(?:|s)$/
     */
    public function theEmbeddedListContainsItems($count)
    {
        $payload = $this->getScopePayload();

        $this->thePropertyIsAnArray('_embedded.items');
        assertCount(
            $count,
            $this->arrayGet($payload, '_embedded.items'),
            "Asserting the embedded list contains [$count] items."
        );
    }
